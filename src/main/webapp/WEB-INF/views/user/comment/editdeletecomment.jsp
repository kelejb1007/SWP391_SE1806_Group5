        ...
        <c:if test="${not empty sessionScope.user}">
            <c:if test="${not empty comment}">
                <form action="comments" method="post">
                    ...
                    <textarea name="commentContent">${comment.content}</textarea>
                    
                    <div class="action-buttons">
                        <button type="submit" name="action" value="edit"> Chỉnh sửa</button>
                        <button type="submit" name="action" value="delete" onclick="return confirm('Bạn có chắc chắn xóa bình luận không?');">Xóa</button>
                    </div>
                </form>
            </c:if>
        </c:if>
